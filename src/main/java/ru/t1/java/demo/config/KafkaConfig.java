...
public class KafkaConfig {
    ...
    @Value("${t1.kafka.consumer.transaction-error-group-id}")
    private String transactionErrorGroupId;
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, AccountCreateDto> accountListenerContainerFactory() {
        return kafkaListenerContainerFactory(accountListenerFactory());
    }

    @Bean
    public ConsumerFactory<String, AccountCreateDto> accountListenerFactory() {
        return consumerListenerFactory(accountGroupId, AccountCreateDto.class);
    }
    ...
    @Bean
    ConcurrentKafkaListenerContainerFactory<String, String> transactionErrorListenerContainerFactory() {
        return kafkaStringListenerContainerFactory(transactionErrorListenerFactory());
    }

    <T> ConcurrentKafkaListenerContainerFactory<String, T> kafkaStringListenerContainerFactory(
            @Qualifier("consumerStringListenerFactory") ConsumerFactory<String, T> consumerFactory) {
        ConcurrentKafkaListenerContainerFactory<String, T> factory = new ConcurrentKafkaListenerContainerFactory<>();
        factoryBuilder(consumerFactory, factory);
        return factory;
    }
    ...
    @Bean
    public ConsumerFactory<String, String> transactionErrorListenerFactory() {
        return consumerStringListenerFactory(transactionErrorGroupId, String.class);
    }

    private <T> ConsumerFactory<String, T> consumerStringListenerFactory(String groupId, Class<T> clazz) {
        Map<String, Object> props = new HashMap<>();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, servers);
        props.put(ConsumerConfig.GROUP_ID_CONFIG, groupId);
        props.put(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, StringDeserializer.class);
        props.put(JsonDeserializer.VALUE_DEFAULT_TYPE, "java.lang."+ clazz.getSimpleName());
        props.put(JsonDeserializer.TRUSTED_PACKAGES, "*");
        props.put(JsonDeserializer.USE_TYPE_INFO_HEADERS, false);
        props.put(ConsumerConfig.SESSION_TIMEOUT_MS_CONFIG, sessionTimeout);
        props.put(ConsumerConfig.MAX_PARTITION_FETCH_BYTES_CONFIG, maxPartitionFetchBytes);
